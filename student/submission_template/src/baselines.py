...
Functionally identical to ``student/baseline/baselines.py`` (the same code the
course leaderboard uses to seed its baseline rows), re-exported here so the
project can score baselines against the same local splits as the neural models
without depending on files outside the submission archive.
"""
...
def make_all_baselines(
    train_frame: pd.DataFrame, forecast_index: pd.DataFrame
) -> dict[str, pd.DataFrame]:
    """Build every reference baseline prediction frame."""
    return {
